# ----------------------------------------------------------------
# Red kit population test without disturbance
# ----------------------------------------------------------------
# This script gives a model of the Redkite population without disturbance 
...
Red_kite_population <- function(x_dim, y_dim, t,
  ...
  # 3.1 assign new coordinates and save them 
  ...
  # 3.2 mortality & new nets & placement of new_coords 
  # 3.2.1 mortality 2, check if new coords are in turbine/buffer, if true - killed
  # 3.2.2 new nest if two kites meet
  # 3.2.3 closest lonely kites build new nests (< new_nest_dist)
  # 3.2.4 movement of rest (set age, abundance at new coords)
  # ---------------------------------------------------------------------------
  ...
  if (n_lonely>0){
    ...
    # 3.2 check assigning new coords ----
    ...
    # 3.2.2 new nest if two kites meet  ----
    # if 3 kites are at the same coordinate -> it stays at the old place
    max_attempts <- 50
    attempt <- 0
    ...
    while(TRUE){
      ...
      is_match <- apply(dup_coords, 1, function(row) {
        any(apply(del_coords, 1, function(dr) all(row == dr)))
      })
      del_idx_help <- which(is_match)
    ...
    # If max attempts were reached and there are still >2 kites at a location, set lonely_age to 0
    if (attempt >= max_attempts) {
    ...
    # 3.2.3 closest lonely kites build new nests (< new_nest_dist) ----
    # closest lonely kites distanc < new_nest_dist will 100% build a nest! 
    # pairwise distance matrix
    remain_kites <- lonely_kites[lonely_kites$nest_age == 0,]
    dist_matrix <- as.matrix(dist(remain_kites[,1:2]))
    
    # Set diagonal to Inf so we don't match points to themselves
    diag(dist_matrix) <- Inf
    
    # set distance > threshold new_nest_dist to NA
    dist_matrix[dist_matrix > new_nest_dist] <- NA
    
    # get coords (row and col names) of values that are not NA
    valid_pairs <- which(!is.na(dist_matrix), arr.ind = TRUE)
    closest_pairs <- data.frame()
    
    if(length(valid_pairs) > 0){
      pair_names <- data.frame(
        from = rownames(dist_matrix)[valid_pairs[, 1]],
        to   = colnames(dist_matrix)[valid_pairs[, 2]],
        distance = dist_matrix[!is.na(dist_matrix)]
      )
      pair_names <- pair_names[order(pair_names$distance), ]  # sort by closest first
      
      # to prevent the build of a new nest with 3 kites
      # data is sort by closest first
      # if coord is already use the third kite remains lonely (on its newest coord)
      used <- c()
      closest_pairs <- data.frame()
      
      for (i in 1:nrow(pair_names)) {
        from <- pair_names$from[i]
        to <- pair_names$to[i]
        
        if (!(from %in% used) && !(to %in% used)) {
          closest_pairs <- rbind(closest_pairs, pair_names[i, ])
          used <- c(used, from, to)
        }
      }
      
      # new nest building for the closest pairs
      new_nest_closest <- data.frame()
      if (nrow(closest_pairs) > 0) {
        for (i in 1:nrow(closest_pairs)){
          new <- remain_kites[c(closest_pairs[i,1], closest_pairs[i,2]),]
          new$nest_age  <- round(mean(new$lonely_age))
          new$lonely_age <- 0
          
          # new nest postion
          # randomly take one of those two coords
          pos <- sample(1:2, 1)
          new <- new[-pos,]
          
          new_nest_closest <- rbind(new_nest_closest, new)
        }
        
        # add new_nests to df_kites
        df_kites <- rbind(df_kites, new_nest_closest)
        
        
        # 3.2.3 check new_nest_dist placement ----
        if(nrow(closest_pairs) != nrow(new_nest_closest)){
          stop("after new_nest_dist closest_pairs unequal new_nest number")
        }
        
        # # Plot all points
        # plot(remain_kites[,1:2], pch = 20, col = "grey", 
        #      main = paste("timestep ",t+1,"Closest Pairs (< threshold) \n blue = new nest postion"), 
        #      xlab = "X", ylab = "Y")
        # apply(closest_pairs, 1, function(row) { # chatGTP
        #   from <- as.character(row["from"])
        #   to   <- as.character(row["to"])
        #   
        #   # Get coords by rowname
        #   if (from %in% rownames(remain_kites[,1:2]) && to %in% rownames(remain_kites[,1:2])) {
        #     coords <- rbind(remain_kites[,1:2][from, ], remain_kites[,1:2][to, ])
        #     lines(coords, col = "red")
        #   }
        # })
        # points(new_nest_closest$x, new_nest_closest$y, col = "blue")
        close_rows <- c(closest_pairs[,1], closest_pairs[,2])
        lonely_kites <- lonely_kites[!(rownames(lonely_kites) %in% close_rows), ]
      }
      
    }
    
    # 3.2.4 movement, write new lonely kites and new nests into df_kites ----
    # delete the rows written in closest_pairs 
    df_kites <- rbind(df_kites, lonely_kites)
    
    # 3.2 check new nests ----
    n_new_nests <- nrow(lonely_kites[lonely_kites$nest_age > 0,]) + nrow(closest_pairs)
    print(paste("3.2.2 after movement:", n_new_nests, "new nests | movement:", nrow(lonely_kites[lonely_kites$nest_age > 0,])))
  ...
  # 4 carrying capacity ----
  # ----------------------------------------------------------------------------
  # if there would be more nest than carrying capacity can hold 
  # random new nest will die
  
  nests <- df_kites[(df_kites$nest_age > 0),]
  empty_nests <- df_kites[(df_kites$nest_age > 0) & (df_kites$juv_age == 0),]
  
  n_nests <- nrow(nests)
  print(paste("4. total nests", n_nests, "| carryCappa:", carrying_capacity))
  if(n_nests > carrying_capacity) {
    dif <- n_nests - carrying_capacity
    print(paste("carrying capacity reached", dif))
    
    # delete random new nests if there are more empty nests than dif
    if (nrow(empty_nests) >= dif){
      print(paste("carrying capacity random"))
      remove_idx <- sample(rownames(empty_nests), dif)
      removed_nests <- empty_nests[(rownames(empty_nests) %in% remove_idx), ]
      
      # delete from df_kites
      df_kites <- df_kites[!(rownames(df_kites) %in% remove_idx), ]
      
      # count as natural death
      real_died_natural <- real_died_natural + nrow(removed_nests)
      
    } else {
      print(paste("carrying capacity all"))
    # delete all empty nests die
      df_kites <- df_kites[!(rownames(df_kites) %in% rownames(empty_nests)), ]
      
      # count as natural death
      real_died_natural <- real_died_natural + nrow(empty_nests)
    }
    
  }
  
  # ----------------------------------------------------------------------------
  # 5 Reproduction of kites ----
  ...
  kites_abund[t+1, ] <- c(t+1, total_abund, abund_nest, abund_juv, abund_lonely, abund_new_juv, n_new_nests,
  ...
  if(sum(kites[,,t+1,"loc"] == 1) != abund_lonely || sum(kites[,,t+1,"loc"] == 2) != (abund_nest - abund_juv) ||
    ...
    print(paste("5. final abund:", sum(kites[,,t+1,"loc"]), "| theo", total_abund))
  }
...
# TEST function ----
# ---------------------------------------
test_Red_kite_population <- function(x_dim, y_dim, t,
                                     region, building_buffer, turbine, buffer,
                                     rep_age, growth_rate, liv_exp, carrying_capacity,
                                     kites_metric, kites_abund){
  ...
  for (t in 1:(timesteps - 1)) {
    ...
    test <- Red_kite_population(x_dim, y_dim, t,
                                region, building_buffer, turbine, buffer,
                                rep_age, growth_rate, liv_exp, carrying_capacity,
                                kites_metric, kites_abund)
    
    kites_abund <- test$kites_abund
    kites_metric <- test$kites_metric
    kites <- test$kites
  }
