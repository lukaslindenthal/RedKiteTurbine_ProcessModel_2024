...
test_Red_kite_population <- function(x_dim, y_dim, timesteps,
                                     rep_age, growth_rate, liv_exp, carrying_capacity,
                                     kites_metric, kites_abund){
  # ---------------------------------------------------------------------------
  # Zero Obstacles and Turbines ----
  # ---------------------------------------------------------------------------
  ...
  for (t in 1:(timesteps - 1)) {
    ...
    df_kites[, 3:5] <- apply(df_kites[, 3:5], 2, function(x) ifelse(x > 0, x + 1, x)) # 2 = column-wise
    ...
    # 3.2.5 movement of rest (set age, abundance at new coords)
    # ---------------------------------------------------------------------------
    ...
    if (n_lonely>0){
      ...
      n_nests <- (nrow(new_nest_closest) + nests)
      if( n_nests > carrying_capacity) {
        dif <- n_nests - carrying_capacity
        print(paste("carrying capacity reached", diff))
        
        # delete random new nests
        remove_idx <- sample(1:nrow(new_nest_closest), dif)
        removed_nests <- new_nest_closest[remove_idx, ]
        
        new_nest_closest <- new_nest_closest[-remove_idx, ]
        
        # count as natural death
        natural_dead_count <- nrow(removed_nests)
      }
      
      # 3.2.5 movement, write new lonely kites and new nests into df_kites ----
